...
describe NonRssFeeds::DowJonesSelect::ArticlesToSectionRssFeeder do
  let(:article_1) do
    {
  ...
  let(:non_rss_feed) do
    NonRssFeed.new(
      name: 'Feed',
      url: 'http://feed.com',
      limits: {'MarketWatch' => 1, 'Barrons' => 2}
    )
  end
  ...
  describe '#xml_rss_feeds' do
    ...
    let(:article_to_rss_feeder_double) do
      instance_double(NonRssFeeds::DowJones::ArticlesToRssFeeder)
    end
    ...
    it 'calls the ArticlesToRssFeeder' do
      expect(article_to_rss_feeder_double)
        ...
        .exactly(2).times
      expect(NonRssFeeds::DowJones::ArticlesToRssFeeder)
        .to receive(:new) do |article_arg, channel_params_arg|
          expect(article_arg).to be_in [[article_1, article_2], [article_3]]
          expect(channel_params_arg[:title]).to be_in ['MarketWatch', "Barron's Online"]
