...
describe NonRssFeeds::Morningstar::MultiRssFeedUploader do
  let(:non_rss_feed) do
  ...
  describe '#upload' do
    let(:xml_rss_feeds) do
      NonRssFeeds::Morningstar::ArticlesToSectionRssFeeder.new(
        [article_1, article_2],
        non_rss_feed
      ).xml_rss_feeds
    end
    let(:rss_feed_uploader_double) do
      instance_double(NonRssFeeds::Morningstar::RssFeedUploader)
    end
    let(:subject) do
    ...
    context 'when the xml_feed is uploaded' do
      it 'it calls the rss feed uploader' do
        expect(NonRssFeeds::Morningstar::RssFeedUploader)
